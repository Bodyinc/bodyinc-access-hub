...
function appOrigin(): string {
  const configured = (process.env.ADMIN_APP_URL || process.env.APP_URL || "")
    .trim()
    .replace(/\/$/, "");
  const vercelHost = (process.env.VERCEL_PROJECT_PRODUCTION_URL || process.env.VERCEL_URL || "")
    ...
    .replace(/^https?:\/\//, "")
    .replace(/\/$/, "");
  const patient = (process.env.PATIENT_PORTAL_URL || "").trim().replace(/\/$/, "");
  ...
  if (patient && !isLocalhost(patient)) return patient;
  return "https://patient.bodyinc.com";
}

/** Hosted PNG of `public/email-logo.png` — Gmail/Outlook do not render SVG in mail. */
...
export function emailButton(label: string, href: string): string {
  ...
  <tr>
    <td align="left" bgcolor="${t.cta}" height="46" class="email-btn-cell" style="background-color:${t.cta};background:${t.cta};border-radius:105px;height:46px;mso-padding-alt:14px 19px;">
      <a href="${safeHref}" target="_blank" class="email-btn" style="display:inline-block;background-color:${t.cta};background:${t.cta};border:1px solid ${t.cta};color:${t.navy};padding:14px 19px;border-radius:105px;text-decoration:none;font-weight:500;font-size:14px;line-height:18px;font-family:${FONT};">
        <!--[if mso]>&nbsp;&nbsp;<![endif]-->${safeLabel}<!--[if mso]>&nbsp;&nbsp;<![endif]-->
      </a>
    </td>
  </tr>
</table>`;
...
export function emailChip(text: string): string {
  ...
  <span class="email-chip" style="display:inline-block;background-color:${t.inputBg};color:${t.navy};border-radius:999px;padding:6px 12px;font-size:12px;line-height:16px;font-weight:500;font-family:${FONT};">${escapeHtml(text)}</span>
</p>`;
}
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${t.page}" class="email-page" style="background-color:${t.page};background:${t.page};padding:32px 16px;">
      <tr><td align="center">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${t.card}" class="email-card" style="max-width:560px;background-color:${t.card};background:${t.card};border-radius:20px;border:1px solid ${t.border};overflow:hidden;">
          ...
          <tr>
            <td class="email-body" bgcolor="${t.card}" style="background-color:${t.card};padding:28px 28px 8px;font-size:24px;font-weight:500;letter-spacing:-0.5px;line-height:1.3;color:${t.navy};font-family:${FONT};">${escapeHtml(title)}</td>
          </tr>
          <tr>
