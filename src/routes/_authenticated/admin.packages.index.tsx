...
function PackagesListPage() {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-2.5">
          ...
          <Button
            ...
            className="bg-[#2A00A2] hover:bg-[#1E0075] text-white font-bold h-11 px-5 rounded-xl shadow-sm transition-colors shrink-0 disabled:opacity-50"
          >
      ...
      {!isEmpty && (
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative min-w-[240px] max-w-sm flex-1">
            <Search className="pointer-events-none absolute left-3.5 top-3.5 h-4 w-4 text-[#6B5AE0]/50" />
            <Input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search packages"
              className="pl-10 h-11 rounded-xl border-[#E2DCFA] focus-visible:ring-[#4A3AFF] focus-visible:ring-1 text-[#2A00A2] font-semibold text-[14px] placeholder:text-[#6B5AE0]/40"
            />
          </div>
          
          <Select value={medicineId} onValueChange={setMedicineId}>
            <SelectTrigger className="w-52 h-11 rounded-xl border-[#E2DCFA] bg-white text-[#2A00A2] font-semibold text-[14px] focus:ring-1 focus:ring-[#4A3AFF] outline-none shadow-none">
              <SelectValue placeholder="All medicines" />
            </SelectTrigger>
            <SelectContent className="rounded-xl border-[#E2DCFA]">
              <SelectItem value="all" className="font-medium text-[#2A00A2]">All medicines</SelectItem>
              {medicines.map((m) => (
          ...
          <Select value={status} onValueChange={(v) => setStatus(v as typeof status)}>
            <SelectTrigger className="w-40 h-11 rounded-xl border-[#E2DCFA] bg-white text-[#2A00A2] font-semibold text-[14px] focus:ring-1 focus:ring-[#4A3AFF] outline-none shadow-none">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="rounded-xl border-[#E2DCFA]">
      ...
      {isEmpty ? (
        <Card className="border-dashed border-[#EAE6FA] bg-white rounded-2xl">
          <CardHeader className="text-center">
            <CardTitle className="text-xl font-bold text-[#2A00A2]">No packages yet</CardTitle>
            ...
            <Table>
              <TableHeader className="bg-[#FBFBFF] border-b border-[#EAE6FA]">
                <TableRow className="hover:bg-transparent border-none">
                  <TableHead className="h-12 font-bold text-[#2A00A2] text-[14px] px-6">Medicine</TableHead>
                  <TableHead className="h-12 font-bold text-[#2A00A2] text-[14px] px-6">Plan</TableHead>
                  <TableHead className="h-12 font-bold text-[#2A00A2] text-[14px] px-6">Duration</TableHead>
                  <TableHead className="h-12 font-bold text-[#2A00A2] text-[14px] px-6">Original</TableHead>
                  <TableHead className="h-12 font-bold text-[#2A00A2] text-[14px] px-6">Sale</TableHead>
                  <TableHead className="h-12 font-bold text-[#2A00A2] text-[14px] px-6">Savings</TableHead>
                  <TableHead className="h-12 font-bold text-[#2A00A2] text-[14px] px-6">Popular</TableHead>
              ...
              <TableBody>
                {query.isLoading && (
                  <TableRow>
                    <TableCell colSpan={9} className="py-12 text-center font-semibold text-[#6B5AE0]/60">
                ...
                {rows.map((p: ListPackageRow) => (
                  ...
                  >
                    ...
                    <TableCell className="font-medium text-[#6B5AE0]/80 text-[14px] py-4 px-6 whitespace-nowrap">
                      {p.duration_months} mo
                    </TableCell>
                    <TableCell className="font-medium text-[#6B5AE0]/50 line-through text-[14px] py-4 px-6">
  ...
  {p.savings > 0 ? (
    <span className="text-[#2A00A2]">
      {formatPrice(p.savings)}
    </span>
  ) : (
    <span className="text-[#6B5AE0]/40">—</span>
  )}
</TableCell>
                    <TableCell className="py-4 px-6">
                      {p.is_most_popular ? (
                        <Badge className="bg-[#4A3AFF] hover:bg-[#4A3AFF] text-white font-bold text-[12px] px-2.5 py-0.5 rounded-lg shadow-none normal-case tracking-normal">
                          Most Popular
                        </Badge>
                      ) : (
                        <span className="text-[#6B5AE0]/40 font-medium text-[14px]">—</span>
                      )}
                    </TableCell>
                    <TableCell className="py-4 px-6">
                      <Badge 
                    ...
                    <TableCell className="py-4 px-6 text-right" onClick={(e) => e.stopPropagation()}>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="rounded-xl border-[#E2DCFA] p-1 shadow-md bg-white">
                          ...
                          <DropdownMenuItem
                            className="rounded-lg font-medium text-[#2A00A2] focus:bg-[#F5F3FF] focus:text-[#2A00A2] cursor-pointer"
                            onClick={() =>
