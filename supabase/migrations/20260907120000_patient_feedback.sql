-- Portal-wide patient feedback (floating widget on the patient app).
-- Inserts go through the service role so guests on auth/onboarding can submit;
-- patients can also insert their own rows. Admins can read everything.
...
DROP POLICY IF EXISTS "patients insert own feedback" ON public.patient_feedback;
CREATE POLICY "patients insert own feedback" ON public.patient_feedback
  FOR INSERT TO authenticated
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "patients read own feedback" ON public.patient_feedback;
CREATE POLICY "patients read own feedback" ON public.patient_feedback
  FOR SELECT TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "admins read all feedback" ON public.patient_feedback;
