...
//#region node_modules/.nitro/vite/services/ssr/assets/medicines-8zD_oRw1.js
...
function createMedicine(values) {
...
function updateMedicine(id, values) {
	const all = sortMedicines(seedIfEmpty());
...
function setMedicineActive(id, status) {
	const all = sortMedicines(seedIfEmpty());
	const index = all.findIndex((m) => m.id === id);
	if (index === -1) throw new Error("Medicine not found");
	all[index] = {
