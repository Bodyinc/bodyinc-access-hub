...
//#region node_modules/.nitro/vite/services/ssr/assets/providers.functions-D0eKqhRT.js
...
var getProvider = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(getProvider_createServerFn_handler, async ({ data, context }) => {
	await assertAdmin(context);
	const { data: row, error } = await context.supabase.from("providers").select("*, profiles!inner(full_name, email, phone, avatar_url)").eq("id", data.id).maybeSingle();
	if (error) throw new Error(error.message);
	if (!row) throw new Error("Provider not found");
	const { profiles, ...provider } = row;
	return {
		...provider,
		...profiles ?? {}
	};
