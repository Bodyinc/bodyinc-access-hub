...
import { r as SEED_IDS, s as listMedicines } from "./medicines-LJTZ9Z5B.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/packages-DxomQ_mP.js
...
async function updatePackage(id, values) {
	const all = sortPackages(seedIfEmpty());
...
async function setPackageActive(id, is_active) {
	const all = sortPackages(seedIfEmpty());
	const index = all.findIndex((p) => p.id === id);
	if (index === -1) throw new Error("Package not found");
	all[index] = {
